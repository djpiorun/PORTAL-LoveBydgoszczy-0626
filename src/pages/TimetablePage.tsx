  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main
        ...
        style={{
          paddingTop: isMobile ? "calc(4.8rem + env(safe-area-inset-top,0px))" : "6rem",
          paddingBottom: isMobile ? "calc(6rem + env(safe-area-inset-bottom,0px))" : "3rem",
        }}
      >
        ...
        <div className={`${isMobile ? "max-w-md" : "max-w-5xl"} mx-auto px-4 sm:px-6 lg:px-8 relative z-10`}>
          {isMobile ? (
            <div className="mb-4 rounded-[1.75rem] border border-border/60 bg-card/95 p-4 shadow-lg backdrop-blur-xl">
              <div className="mb-2 flex items-center justify-between">
                <h1 className="text-xl font-black text-foreground">Rozkład jazdy</h1>
                <button
                  onClick={() => navigate(-1)}
              ...
              <p className="text-sm text-muted-foreground">
                Linie tramwajowe, autobusy i szybkie wyszukiwanie przystanków.
              </p>
            </div>
          ) : (
            <TimetableHero />
          ...
          {routes && routes.length === 0 && <TimetableEmptyState />}

          {routes && routes.length > 0 && (
            <div className="space-y-4 sm:space-y-6">
              <RouteCategory title="Tramwaje" subtitle="Linie dzienne" icon={TrainFront} iconColorClass="text-red-500" iconBgClass="bg-red-50" routes={trams} hoverBgClass="hover:bg-red-500" hoverTextClass="hover:text-white" hoverBorderClass="hover:border-red-500" />
              <RouteCategory title="Autobusy miejskie" subtitle="Linie dzienne" icon={Bus} iconColorClass="text-blue-500" iconBgClass="bg-blue-50" routes={dayBuses} hoverBgClass="hover:bg-blue-500" hoverTextClass="hover:text-white" hoverBorderClass="hover:border-blue-500" />
              <RouteCategory title="Autobusy międzygminne" subtitle="Linie podmiejskie" icon={MapIcon} iconColorClass="text-green-500" iconBgClass="bg-green-50" routes={intercityBuses} hoverBgClass="hover:bg-green-500" hoverTextClass="hover:text-white" hoverBorderClass="hover:border-green-500" />
              <RouteCategory title="Autobusy nocne" subtitle="Linie nocne" icon={Moon} iconColorClass="text-white" iconBgClass="bg-slate-800" routes={nightBuses} hoverBgClass="hover:bg-slate-800" hoverTextClass="hover:text-white" hoverBorderClass="hover:border-slate-800" />
            </div>
          )}

          <div className="flex flex-col items-center gap-2 text-center mt-12 pb-4">
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Rozkład Jazdy dostępny dzięki ZDMiKP w Bydgoszczy.</p>
            {metadata && (
              <div className="text-xs text-slate-600">
                Stan danych: <span className="font-bold">{new Date(metadata.last_update).toLocaleDateString('pl-PL', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            )}
          </div>
        </div>
