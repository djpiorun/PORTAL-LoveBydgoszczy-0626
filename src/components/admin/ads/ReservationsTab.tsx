...
export function ReservationsTab() {
  ...
  const upcoming = campaigns
    .filter((c: any) => c.status === "planned" || c.status === "active" || c.status === "draft")
    .sort((a: any, b: any) => (a.startDate ?? 0) - (b.startDate ?? 0));

  const finished = campaigns
    .filter((c: any) => c.status === "finished")
    .sort((a: any, b: any) => (b.endDate ?? 0) - (a.endDate ?? 0))
    .slice(0, 5);

  const totalBudget = upcoming.reduce((sum: number, c: any) => sum + (c.budget || 0), 0);
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{campaigns.filter((c: any) => c.status === "finished").length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {upcoming.length > 0 ? upcoming.map((c: any) => {
              ...
              return (
                <div key={c._id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50 transition-colors">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {finished.map((c: any) => (
                <div key={c._id} className="flex items-center justify-between p-3 border rounded-lg bg-slate-50 text-sm">
